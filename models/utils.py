...
class FrozenBatchNorm2d(nn.Module):
    """
    FrozenBatchNorm2d 类是一个自定义的批量归一化层（Batch Normalization Layer），
    其特点是 冻结（固定） 了批量归一化的统计量（均值和方差）以及学习到的参数（权重和偏置），这些参数不会在训练过程中更新。这种实现通常用于以下几种情况：
    使用已经训练好的模型（如迁移学习），希望固定批量归一化层的统计量。
    在推理阶段使用，避免每次计算时更新批量归一化的统计量。
    """
    def __init__(self, n):
...
class ConvBNReLU(nn.Module):
    """
    ConvBNReLU 类是一个组合层，通过简单的配置实现了常见的卷积-归一化-激活功能。它的灵活性在于支持使用不同类型的批量归一化（标准或冻结）和是否应用激活函数，
    此外，还支持残差连接，适用于深度神经网络中的多种任务。
    """
