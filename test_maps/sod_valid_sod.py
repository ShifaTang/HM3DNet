...
def main():
    ...
    # test_datasets = ["DUT-RGBD",'ReDwebTest']
    # test_datasets = ["NJU2K"]
    #test_datasets = ["NJU2K","DUT-RGBD","NLPR", "ReDwebTest",  "SIP","STERE"]
    ...
    for dataset in test_datasets:
        ...
        for gt_name in tqdm(gt_name_list, total=len(gt_name_list)): # gt_name_list 是包含所有真实标签图像文件名的列表。
            ...
            FM.step(pred=pred, gt=gt) # 每个函数都会比较预测结果 pred 和真实标签 gt，并更新相应的统计信息。这里只是统计一次也就是一个pred，gt的指标
        ...
        fm = FM.get_results()[0]['fm'] # 获得整个数据集的最终指标
        ...
        meanFm_r = str(fm['curve'].mean().round(4)) # mean FM  是从一系列F_p  取出来的
        maxFm_r = str(fm['curve'].max().round(4)) # max FM
