...
这个评估指标文件和下面那个差不多，这个还多了一些没用到的东西
@author: taozhou
"""
...
###############################################################################
## basic funcs
# fun_eval_e 和 fun_eval_pr 用于评估预测结果的质量，包括对齐度、精度和召回率等。
# fun_S_object 和 fun_object 用于评估分割的前景和背景区域的质量。
# fun_S_region 和 fun_centroid 用于区域级别的评估，特别是基于目标区域划分的分割质量。
# fun_divideGT 和 fun_dividePrediction 用于将图像分为多个区域并计算区域级别的评价指标。
# fun_ssim 用于计算图像间的结构相似度，作为评估分割质量的依据。
###############################################################################
...
###############################################################################
## metric funcs
# eval_mae：用于计算平均绝对误差（MAE），评估模型的预测误差。
# eval_Smeasure：用于计算S-measure，评估图像分割的质量，特别是前景和背景区域的质量。
# eval_fmeasure：用于计算F-measure，综合考虑分割的精度和召回率，评估二分类任务中的分割质量。
###############################################################################
...
class Eval_thread():
    """
    这段代码定义了一个多线程评估类 Eval_thread，用于在图像分割任务中评估模型的不同指标（例如MAE、S-measure、F-measure等）。
    该类通过加载数据、计算各项指标并将结果记录到日志中来评估模型性能。
    """
