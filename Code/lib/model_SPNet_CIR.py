...
"""
这个文件结合SPNet论文的CMI,MFA,RFB模块,同时结合了CIR的模块
"""
...
class SAINet_SPNet_CIR(nn.Module):
    """
    模型结构类
    """
    def __init__(self, channel=32, ind=50, resnet=None):
        super(SAINet_SPNet_CIR, self).__init__()
        ...
        self.conv1 = BaseConv2d(2 * 128, 128, kernel_size=1)
        self.conv2 = BaseConv2d(2 * 256, 256, kernel_size=1)
        self.conv3 = BaseConv2d(2 * 512, 512, kernel_size=1)
    ...
    def forward(self, imgs, depths, img_depths):
        ...
        # 加上cmWR模块 progressive attention guided integration unit，下面是rgb-d融合的特征
        B, C, H, W = img_4.size()
        P = H * W
        conv3_rgbd = self.conv1(torch.cat((img_2, dep_2), dim=1))
        conv3_rgbd = F.interpolate(conv3_rgbd, scale_factor=1/2, mode='bilinear', align_corners=True)
        conv3_rgbd_map = self.sa1(conv3_rgbd)
        
        conv4_rgbd = self.conv2(torch.cat((img_3, dep_3), dim=1))
        ...
        conv5_rgbd = self.conv3(torch.cat((img_4, dep_4), dim=1))
        ...
        # self-modality attention refinement，这里去掉了原论文中的得到卷积结果之后，再对结果进行一次卷积的操作
        B, C, H, W = img_4.size()
        P = H * W

        rgb_SA = self.sa_rgb(img_4).view(B, -1, P)            # B * 1 * H * W
        depth_SA = self.sa_depth(dep_4).view(B, -1, P)

        rgb_CA = self.ca_rgb(img_4).view(B, C, -1)           # B * C * 1 * 1
        depth_CA = self.ca_depth(dep_4).view(B, C, -1)

        rgb_M = torch.bmm(rgb_CA, rgb_SA).view(B, C, H, W)
        depth_M = torch.bmm(depth_CA, depth_SA).view(B, C, H, W)

        rgb_smAR = img_4 *  rgb_M + img_4 
        depth_smAR = dep_4 * depth_M + dep_4 
