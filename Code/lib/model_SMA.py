...
from SMA.ImageDepthNet import NonLocalBlock
"""
这个文件结合SPNet论文的CMI,MFA,RFB模块，同时结合SMA结构
...
class self_RD3D(nn.Module):
    ...
    def forward(self, x):
        """
        I3DResNet 中，x0, x1, x2, x3, x4 的通道数与 ResNet 的基础结构一致，
        因为膨胀操作（inflate_conv 和 inflate_batch_norm）只是将 2D 卷积扩展为 3D 卷积，通道数保持不变。
        """
        # 下面的是img，depth的融合图像
        x = self.resnet.conv1(x) # 利用二维卷积的参数，加上时间维度，创建三维卷积
        x = self.resnet.bn1(x)
        ...
        x_s0 = self.reductions0(x0) # 特征提取
        x_t_0 = self.T_to_0(x_s0).squeeze(2) # 时间维度降维
...
class Res_part(nn.Module):
    ...
    def forward(self, MFIB_res, RDD3D_res):
        ImageFlow_Fea = torch.cat([MFIB_res, RDD3D_res], dim=1)
        ImageFlow_resFea = self.res_bn_relu(ImageFlow_Fea)
        ImageFlow_resFea = self.res_conv(ImageFlow_resFea)
        res1 = self.BR(MFIB_res + ImageFlow_resFea) # 拼接之后归一化
        res2 = self.CBR(res1) # CBR操作

        return res2

# Dense ASPP模块的组件，用来更好地提取图像的特征
...
class DASPPmodule(nn.Module):
    def __init__(self):
        ...
        self.AvgPool = nn.Sequential(
            ...
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.Upsample(size=8, mode='nearest'),
        )
...
class SAINet_SPNet_SMA(nn.Module):
    """
    模型结构类
    """
    def __init__(self, channel=32, ind=50, resnet=None):
        super(SAINet_SPNet_SMA, self).__init__()
        ...
        # S2MA module
        self.NonLocal = NonLocalBlock(in_channels=512)  # 输入通道数是512
        ...
        # 加上融合MFIB的结果和三维卷积结果的残差结构
        self.res_part4 = Res_part(in_channels=256)
        self.res_part3 = Res_part(in_channels=128 + 32)
        self.res_part2 = Res_part(in_channels=64 + 32)
        self.res_part1 = Res_part(in_channels=64 + 32)
        self.res_part0 = Res_part(in_channels=64 + 32)
    ...
    def forward(self, imgs, depths, img_depths):
        ...
        # DASPP 模块，更好地提取图像的特征
        img_4 = self.ImageBranch_DASPP(self.image_bn_relu(img_4))  # 获取resnet卷积的最后的输出，调整通道，输入DASPP模块
        dep_4 = self.DepthBranch_DASPP(self.depth_bn_relu(dep_4))
        
        
        # 就在这里开始使用SMA模块，进一步融合img4和dep4的特征，使之向前传播
        
        bs, ch, hei, wei = img_4.size()
        affinityAtt = F.softmax(self.affinityAttConv(torch.cat([img_4, dep_4], dim=1)))
        ...
        ImageAfterAtt1 = self.NonLocal(img_4, dep_4, alphaD, selfImage=True)
        DepthAfterAtt1 = self.NonLocal(dep_4, img_4, alphaR, selfImage=False)
        img_4 = self.image_bn_relu(ImageAfterAtt1) # 使用SMA模块之后的rgb图像数据,重新赋值为img4
        dep_4 = self.depth_bn_relu(DepthAfterAtt1)
        ...
        x_ful_42 = self.res_part4(x_ful_42, x_t_4) # 这里使用残差结构，融合MFIB的结果和三维卷积的结果
        ...
        x_ful_32 = self.res_part3(x_ful_32, x_t_3)
        ...
        x_ful_22 = self.res_part2(x_ful_22, x_t_2)
        ...
        x_ful_12 = self.res_part1(x_ful_12, x_t_1)
        ...
        x_ful_02 = self.res_part0(x_ful_02, x_t_0)
