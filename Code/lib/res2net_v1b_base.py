...
class Res2Net(nn.Module):
    """
    是一个标准的深度神经网络，带有最终的全连接层和分类输出，适用于分类任务。
    """
...
class Res2Net_Ours(nn.Module):
    """
    扩展或修改版，主要的区别是它不包含分类层，返回了网络的多个中间层输出，适用于需要多个中间特征图的任务（例如特征提取、分割等）
    """
    def __init__(self, block, layers, baseWidth = 26, scale = 4, num_classes=1000):
        self.inplanes = 64
...
def res2net50_v1b(pretrained=False, **kwargs):
    """
    函数功能：构建并返回一个 Res2Net-50_v1b 模型。
    参数说明：
    pretrained=False：是否加载预训练的权重。
    **kwargs：额外参数传递给 Res2Net。
    Res2Net-50_v1b 结构：
    基于 Bottle2neck 模块。
    层数配置为 [3, 4, 6, 3]。
    
    baseWidth=26：表示每个分支的基础宽度为 26 通道。
    scale=4：表示在 Bottle2neck 块中分为 4 个分支。
    
    预训练权重：如果 pretrained=True，会从远程加载预训练权重（通常来自 ImageNet 数据集）。
    """
