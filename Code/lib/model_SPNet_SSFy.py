...
class CAAF(nn.Module):
    """
    CAAF 模块的核心思想是利用空间注意力图和通道注意力图，结合深度特征图和 RGB 图像特征图，生成一个融合特征图，
    """
    ...
    def forward(self, rgb, depth):
        ...
        # 四个张量沿着维度 1 进行拼接。这里的维度 1 通常是指 通道维度，即将这些张量的通道数拼接起来。
...
class MFIB(nn.Module):
    ...
    def forward(self, x_ful, x_rgb, x_dep):  # 参数分别是CAAF模块输出的数据，rgb数据，depth数据
...
class rfb_reduce(nn.Module):
    """
    仅仅用来减少维度的
    """
    def __init__(self, in_channel, out_channel):
        super(rfb_reduce, self).__init__()
        self.decoder = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=1, padding=1),
            # nn.BatchNorm2d(out_channel),
            # nn.ReLU(inplace=True),
...
class SAINet_SPNet(nn.Module):
    ...
    def __init__(self, channel=32, ind=50, resnet=None):
        ...
        self.rfb_r4 = rfb_reduce(512, 256)
        self.rfb_r3 = rfb_reduce(256, 128)
        self.rfb_r2 = rfb_reduce(128, 64)
        self.rfb_r1 = rfb_reduce(64, 64)
        self.rfb_r0 = rfb_reduce(64, 64)
    ...
    def forward(self, imgs, depths, img_depths):
        ...
        # MFIB+Decoder+Pre 
        ###############################################
        RFB_img4 = self.RFB4(img_4)
        boundary_4 = self.rfb_r4(dep_4.mul(RFB_img4) + dep_4) 
        
        x_ful_42 = self.mfib_layer4(ful_4, x_rgb_42, x_dep_42) + boundary_4
        ...
        RFB_img3 = self.RFB3(img_3)
        boundary_3 = self.rfb_r3(dep_3.mul(RFB_img3) + dep_3)
        
        x_ful_32 = self.mfib_layer3(ful_3, x_rgb_32, x_dep_32) + boundary_3
        ...
        RFB_img2 = self.RFB2(img_2)
        boundary_2 = self.rfb_r2(dep_2.mul(RFB_img2) + dep_2)

        x_ful_22 = self.mfib_layer2(ful_2, x_rgb_22, x_dep_22) + boundary_2
        ...
        RFB_img1 = self.RFB1(img_1)
        boundary_1 = self.rfb_r1(dep_1.mul(RFB_img1) + dep_1)
        
        x_ful_12 = self.mfib_layer1(ful_1, x_rgb_12, x_dep_12) + boundary_1
        ...
        RFB_img0 = self.RFB0(img_0)
        boundary_0 = self.rfb_r0(dep_0.mul(RFB_img0) + dep_0)
        
        x_ful_02 = self.mfib_layer0(ful_0, x_rgb_02, x_dep_02) + boundary_0
