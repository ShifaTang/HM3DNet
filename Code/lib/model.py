...
# 这个类是用来代替模型上面一层的decoder，更好地融合特征的
class AdvancedFusion(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        # 输出卷积层，调整输出通道数
        self.output_conv = nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=1, padding=1)

    def forward(self, data1, data2):
        # 通道注意力
        attention_map = self.channel_attention(data1 + data2)  # 可以选择加法操作
        data1 = data1 * attention_map
        data2 = data2 * attention_map

        # 空间注意力
        concatenated = torch.cat([data1, data2], dim=1)  # 在通道维度拼接
        spatial_attention_map = self.spatial_attention(concatenated)
        spatial_attention_map = torch.sigmoid(spatial_attention_map)
        fused_data = data1 * spatial_attention_map + data2 * (1 - spatial_attention_map)

        # 调整输出通道数
        fused_data = self.output_conv(fused_data)
...
class SAINet(nn.Module):
    ...
    def forward(self, imgs, depths, img_depths):
        ...
        x_dep_32 = self.dep_gcm_3(torch.cat((dep_3, self.upsample_2(x_dep_42)), dim=1))
        x_dep_22 = self.dep_gcm_2(torch.cat((dep_2, self.upsample_2(x_dep_32)), dim=1))
        x_dep_12 = self.dep_gcm_1(torch.cat((dep_1, self.upsample_2(x_dep_22)), dim=1))
        x_dep_02 = self.dep_gcm_0(torch.cat((dep_0, self.upsample_2(x_dep_12)), dim=1))
