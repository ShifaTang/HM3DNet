...
class Unet3D(nn.Module):
    def __init__(self, channel):
        ...
        self.reductions0 = Reduction3D(64, 64)
        self.reductions1 = Reduction3D(256, 64)
        self.reductions2 = Reduction3D(512, 64)
        self.reductions3 = Reduction3D(1024, 128)
        self.reductions4 = Reduction3D(2048, 256)
    ...
    def forward(self, x0, x1, x2, x3, x4):
        ...
        print("x_s0 shape:", x_s0.shape)
        print("x_s1 shape:", x_s1.shape)
        print("x_s2 shape:", x_s2.shape)
        print("x_s3 shape:", x_s3.shape)
        print("x_s4 shape:", x_s4.shape)
        
        
        print("x_t_0 shape:", x_t_0.shape)
        print("x_t_1 shape:", x_t_1.shape)
        print("x_t_2 shape:", x_t_2.shape)
        print("x_t_3 shape:", x_t_3.shape)
        print("x_t_4 shape:", x_t_4.shape)
        # x_s0 shape: torch.Size([16, 32, 2, 128, 128])
        # x_s1 shape: torch.Size([16, 32, 2, 64, 64])
        # x_s2 shape: torch.Size([16, 32, 2, 32, 32])
        # x_s3 shape: torch.Size([16, 32, 2, 16, 16])
        # x_s4 shape: torch.Size([16, 32, 2, 8, 8])
...
class RD3D(nn.Module):
    ...
    def forward(self, x):
        size = x.size()[3:]  # 这里的x，最开始已经经过了dataloader了，从 channel h w 变成了 batch channel h w 再到 batch channel t h w
        ...
        print("x0, x1, x2, x3, x4 size is ",x0.shape, x1.shape, x2.shape, x3.shape, x4.shape)
# x0, x1, x2, x3, x4 size is  torch.Size([16, 64, 2, 128, 128]) torch.Size([16, 256, 2, 64, 64]) torch.Size([16, 512, 2, 32, 32])
# torch.Size([16, 1024, 2, 16, 16]) torch.Size([16, 2048, 2, 8, 8])
