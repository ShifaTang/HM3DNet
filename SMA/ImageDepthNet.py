...
class ImageDepthNet(nn.Module):
    ...
    def forward(self, image_Input, depth_Input):
        ...
        ImageAfterDASPP = self.ImageBranch_DASPP(self.ImageBranch_fc7_1(image_feas[-1]))  # 获取Vgg卷积的最后的输出，调整通道，输入DASPP模块

        depth_feas = self.DepthBranchEncoder(depth_Input)  # depth各个阶段的输出
        ...
        outputs_image, outputs_depth = self.ImageBranchDecoder(image_feas, ImageAfterAtt, depth_feas, DepthAfterAtt) # image，depth各个阶段的输出，以及经过S2MA模块之后的输出
