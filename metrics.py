...
def _prepare_data(pred: np.ndarray, gt: np.ndarray) -> tuple:
    # 灰度图像中，黑色的值通常为0，白色的值通常为255或者最大，因此这样可以将灰度图转换为0，1二值
    gt = gt > 128 # 它将标签转化为二值化图像，分为前景与后景
    pred = pred / 255
    if pred.max() != pred.min():
        pred = (pred - pred.min()) / (pred.max() - pred.min()) # 将预测结果归一化到 [0, 1] 的范围内。
    return pred, gt


def _get_adaptive_threshold(matrix: np.ndarray, max_value: float = 1) -> float:
    # 根据输入矩阵的平均值计算一个自适应的阈值，确保图像处理或分割算法中的阈值能够根据数据动态调整，避免固定阈值的限制。
...
class Fmeasure_and_FNR(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        """
        这个方法接收预测结果 pred 和真实标签 gt，并计算各种指标，包括自适应 F-measure (adaptive_fm)、精度 (precisions)、召回率 (recalls)、FNR (fnrs) 
        和可变 F-measure (changeable_fms)。
        """
    ...
    def cal_adaptive_fm(self, pred: np.ndarray, gt: np.ndarray) -> float:
        """
        计算自适应 F-measure（adaptive F-measure），它通过一个自适应阈值（由 _get_adaptive_threshold 函数计算）将预测值转化为二进制形式，
        然后计算交集区域的精度和召回率，并根据 F-measure 公式计算结果。
        """
        adaptive_threshold = _get_adaptive_threshold(pred, max_value=1)
        binary_predcition = pred >= adaptive_threshold # 在此之前pred已经被归一化到 [0, 1] 范围内
        ...
        if area_intersection == 0:
            ...
            adaptive_fm = (1 + self.beta) * pre * rec / (self.beta * pre + rec) # 这个不是F-measure，只是计算出一个自适应的阈值。
        return adaptive_fm

    def cal_pr(self, pred: np.ndarray, gt: np.ndarray) -> tuple:
        """
        首先，将预测值（pred）转换为 8 位无符号整数（0-255）。然后根据预测结果和真实标签计算前景和背景的直方图，并累积求和得到真正例（TP）和总预测结果（P）。
        """
        pred = (pred * 255).astype(np.uint8)
        bins = np.linspace(0, 256, 257)
        fg_hist, _ = np.histogram(pred[gt], bins=bins)  # histogram方法返回 hist：返回每个箱子内的元素数量，bin_edges：返回每个箱子的边界。
