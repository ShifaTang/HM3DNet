...
from Code.utils.options import opt as opt2
"""
/home/accv/tsf/train_val_checkpoints/SPNet_epoch_best_05764.pth
...
model = SAINet(32, 50, copy.deepcopy(RD3D_resnet))
model.cuda()
#checkpoint='./Checkpoint/SAINet_nju_best.pth'  # 之前训练得到的最好的模型
checkpoint='./Checkpoint/SAINet_best.pth'
model.load_state_dict(torch.load(checkpoint))
model.eval()


# val_image_root = opt2.test_rgb_root
# val_gt_root = opt2.test_gt_root
# val_depth_root = opt2.test_depth_root
# save_path = opt2.save_path  # 保存模型的路径
...
# val(test_loader, model, 0, "")
# test
#test_datasets = ['NJU2K','NLPR', 'SIP', 'STERE','DUT-RGBD','ReDWebTest',]  # 六个额外的数据集
...
for dataset in test_datasets:
    ...
    test_loader = test_dataset(image_root, gt_root, depth_root, opt.testsize) # 这里没有继承data.dataloader类，一个批次就是一张图片
    time_sum = 0
    # mae_sum = 0
    for i in range(test_loader.size):
        image, gt, depth, name, image_for_post = test_loader.load_data() # 得到一张图片

        gt = np.asarray(gt, np.float32) # 转换成float32数组
        gt /= (gt.max() + 1e-8) # 归一化
        image = image.cuda()
        depth = depth.repeat(1, 3, 1, 1).cuda()
        ...
        pre_res = model(image, depth, img_depth) # 这里还需要看一下，为什么depth要弄成三通道的，还有这里的pred[0],[1]分别表示什么
        time_end = time.time()
